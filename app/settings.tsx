...
import BattleLayoutLabSection from '@/components/settings/BattleLayoutLabSection';
...
export default function SettingsScreen() {
  ...
  // ── Search / structure ──────────────────────────────────────────────────────
  // Every section + row's title/subtitle/keywords lives in ONE manifest below,
  // instead of scattered across ~1200 lines of JSX. That's what makes the search
  // bar possible, and it's also what makes a future locale swap tractable: each
  // string sits in exactly one place instead of being hunted through markup.
  // Rows that reduce to "icon + label + one control" use the generic SettingsRow
  // (switch/text/link/value); genuinely bespoke bodies (diagnostics boxes, list
  ...
  const sections: SettingsSectionDef[] = [
    {
      id: 'interfejs', title: 'Interfejs', icon: LucideIcons.Palette, color: '#A78BFA',
      keywords: ['ui', 'wygląd', 'wibracje', 'animacje'],
      items: [
        {
          id: 'haptics', title: 'Wibracje', subtitle: 'Haptyczne potwierdzenia przycisków',
          icon: Vibrate, accentColor: colors.accent.purple,
    ...
    },
    {
      ...
      items: [
        ...
        {
          id: 'work-salary', title: 'Ostatnia wypłata (zł)',
          subtitle: workDiag?.hasSalaryOvr
    ...
    {
      id: 'saldo', title: 'Saldo konta', icon: LucideIcons.Wallet, color: '#5B7BE3', defaultOpen: false,
      keywords: ['konto', 'karta', 'bilans'],
      items: [
        {
          id: 'balance', title: 'Ile mam na karcie',
          subtitle: `App liczy teraz: ${(balanceOffset + accountNet.net).toFixed(2)} zł.\nWpisz ile masz realnie na karcie — reszta liczy się sama.`,
          icon: Wallet, accentColor: '#5B7BE3',
          keywords: ['saldo', 'konto', 'karta', 'bilans', 'ile mam pieniędzy', 'korekta'],
          control: { kind: 'text', value: balanceInput, onChangeText: setBalanceInput, onBlur: saveAccountBalance, keyboardType: 'numeric', placeholder: 'np. 230,50', width: 90 },
        },
      ],
    },
    {
      id: 'wyplata', title: 'Wypłata', icon: LucideIcons.Banknote, color: '#2AC68F', defaultOpen: false,
      ...
      items: [
        ...
        ...(paydayEnabled ? [{
          id: 'payday-day', title: 'Którego dnia miesiąca',
          keywords: ['dzień', 'miesiąc', 'data wypłaty'],
          control: {
            kind: 'custom' as const, render: () => (
    ...
    },
    {
      ...
      items: [
        ...
        ...(notifEnabled ? [
              ...
              <>
                ...
                <View style={styles.timeRow}>
                  <View style={styles.timeField}>
                    <InputField value={eveningHour} onChangeText={setEveningHour} placeholder="20" keyboardType="number-pad" containerStyle={{ flex: 1 }} style={styles.timeInput} maxLength={2} />
                    <Text style={styles.timeSep}>:</Text>
                    <InputField value={eveningMin} onChangeText={setEveningMin} placeholder="00" keyboardType="number-pad" containerStyle={{ flex: 1 }} style={styles.timeInput} maxLength={2} />
                  </View>
                </View>
              </>
            ) },
            ...
            control: { kind: 'custom' as const, render: () => (
              <>
                <View style={[styles.row, { paddingTop: 0 }]}>
                  <View style={styles.iconWrap}><Sun size={14} color={colors.text.secondary} /></View>
                  <View style={styles.rowText}>
                ...
                {morningEnabled && (
                  <View style={styles.timeRow}>
                    <View style={styles.timeField}>
                      <InputField value={morningHour} onChangeText={setMorningHour} placeholder="8" keyboardType="number-pad" containerStyle={{ flex: 1 }} style={styles.timeInput} maxLength={2} />
                      <Text style={styles.timeSep}>:</Text>
                      <InputField value={morningMin} onChangeText={setMorningMin} placeholder="00" keyboardType="number-pad" containerStyle={{ flex: 1 }} style={styles.timeInput} maxLength={2} />
                    </View>
                  </View>
                )}
              </>
          ...
          {
            id: 'notif-todo', title: 'Lista zadań', subtitle: 'Dziś, jutro i bezterminowe',
            keywords: ['lista zadań', 'briefing', 'zadania dnia'],
            control: { kind: 'custom' as const, render: () => (
              <>
                <View style={[styles.row, { paddingTop: 0 }]}>
                  <View style={styles.iconWrap}><ClipboardList size={14} color={colors.text.secondary} /></View>
                  <View style={styles.rowText}>
                ...
                {briefingEnabled && (
                  <View style={styles.timeRow}>
                    <View style={styles.timeField}>
                      <InputField value={briefingHour} onChangeText={setBriefingHour} placeholder="7" keyboardType="number-pad" containerStyle={{ flex: 1 }} style={styles.timeInput} maxLength={2} />
                      <Text style={styles.timeSep}>:</Text>
                      <InputField value={briefingMin} onChangeText={setBriefingMin} placeholder="30" keyboardType="number-pad" containerStyle={{ flex: 1 }} style={styles.timeInput} maxLength={2} />
                    </View>
                  </View>
                )}
              </>
          ...
          {
            id: 'notif-habit', title: 'Przypomnienie o nawykach', subtitle: 'Wieczorne przypomnienie o nawykach',
            keywords: ['nawyki', 'wieczorne przypomnienie nawyków', 'habit'],
            control: { kind: 'custom' as const, render: () => (
              <>
                <View style={[styles.row, { paddingTop: 0 }]}>
                  <View style={styles.iconWrap}><Zap size={14} color={colors.accent.amber} /></View>
                  <View style={styles.rowText}>
                ...
                {habitNotifEnabled && (
                  <View style={styles.timeRow}>
                    <View style={styles.timeField}>
                      <InputField value={habitHour} onChangeText={setHabitHour} placeholder="21" keyboardType="number-pad" containerStyle={{ flex: 1 }} style={styles.timeInput} maxLength={2} />
                      <Text style={styles.timeSep}>:</Text>
                      <InputField value={habitMin} onChangeText={setHabitMin} placeholder="00" keyboardType="number-pad" containerStyle={{ flex: 1 }} style={styles.timeInput} maxLength={2} />
                    </View>
                  </View>
                )}
              </>
          ...
          {
            id: 'notif-save', title: 'Zapisz przypomnienia',
            keywords: ['zapisz przypomnienia', 'zastosuj'],
            control: { kind: 'custom' as const, render: () => (
        ...
        ] : []),
        {
          ...
          keywords: ['anuluj', 'wyczyść', 'wszystkie powiadomienia', 'reset'],
          control: { kind: 'custom', render: () => (
    ...
    {
      id: 'dane', title: 'Dane', icon: LucideIcons.Database, color: '#46B0DE', defaultOpen: false,
      keywords: ['statystyki', 'ile', 'liczba wpisów'],
      items: [
        { id: 'data-mood-entries', title: 'Wpisy nastroju', icon: Smile, keywords: ['nastrój', 'wpisy', 'ile wpisów'], control: { kind: 'value', text: String(moodEntries.length) } },
        { id: 'data-transactions', title: 'Transakcje', icon: Database, keywords: ['transakcje', 'wydatki', 'przychody', 'ile'], control: { kind: 'value', text: String(expenses.length) } },
        { id: 'data-tasks', title: 'Zadania', icon: ListTodo, keywords: ['zadania', 'taski', 'ile zadań'], control: { kind: 'value', text: String(tasks.length) } },
        { id: 'data-events', title: 'Wydarzenia', icon: CalendarDays, keywords: ['wydarzenia', 'kalendarz', 'eventy'], control: { kind: 'value', text: String(events.length) } },
      ],
    },
    {
      id: 'konto', title: 'Konto', icon: LucideIcons.User, color: '#2AC68F', defaultOpen: false,
      keywords: ['login', 'synchronizacja'],
      items: [
        {
          id: 'account-google', title: 'Google',
          keywords: ['google', 'konto', 'logowanie', 'synchronizacja', 'login', 'zaloguj'],
          control: { kind: 'custom', render: () => (
            googleUser ? (
    ...
    {
      id: 'personalizacja', title: 'Personalizacja', icon: LucideIcons.Sparkles, color: '#F472B6', defaultOpen: false,
      keywords: ['motyw', 'dashboard'],
      items: [
        {
          id: 'theme-mode', title: 'Motyw', subtitle: 'Jasny/ciemny — wdrażany ekran po ekranie',
          keywords: ['motyw', 'jasny', 'ciemny', 'system', 'theme', 'tryb'],
          control: { kind: 'custom', render: () => (
            <View style={[styles.row, { flexDirection: 'column', alignItems: 'stretch', gap: spacing[2] }]}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing[3] }}>
        ...
        {
          id: 'dashboard-edit', title: 'Edytuj dashboard', subtitle: 'Przesuwaj kafelki, ukrywaj, dodawaj widgety statystyk',
          icon: LucideIcons.LayoutDashboard, accentColor: '#6C9EFF',
          keywords: ['dashboard', 'kafelki', 'widgety', 'edytuj', 'układ', 'sekcje'],
          control: { kind: 'link', onPress: () => { useDashboardLayout.getState().requestEdit(); router.push('/(tabs)' as any); } },
        },
      ],
    },
    {
      ...
      items: [
        {
          ...
          icon: LucideIcons.Trophy, accentColor: '#FFC83D',
          keywords: ['osiągnięcia', 'odznaki', 'gablota', 'achievement', 'trofea', 'serie', 'grzeszki'],
          control: { kind: 'link', onPress: () => { haptic.tap(); router.push('/achievements' as any); } },
        },
        {
          id: 'more-counters', title: 'Liczniki i odliczania', subtitle: 'Odliczanie do wydarzeń, „ile dni temu…"',
    ...
    {
      id: 'bank', title: 'Auto-wydatki z banku (PeoPay)', icon: LucideIcons.Landmark, color: '#2AC68F', defaultOpen: false,
      keywords: ['bank', 'pekao', 'peopay', 'pko', 'iko', 'powiadomienia', 'notification listener', 'nasłuch'],
      items: [
        {
          id: 'bank-enable', title: 'Czytaj powiadomienia o płatności',
          subtitle: `Powiadomienie z banku → wydatek do zatwierdzenia${bankPending > 0 ? ` · ${bankPending} czeka` : ''}`,
          icon: LucideIcons.Landmark, accentColor: '#2AC68F',
        ...
        ...(bankEnabled ? [{
          id: 'bank-test', title: 'Test odczytu powiadomień',
          keywords: ['test', 'diagnostyka', 'wklej powiadomienie', 'sprawdź teraz', 'płatności do zatwierdzenia'],
          control: { kind: 'custom' as const, render: () => (
    ...
    {
      id: 'diagnostyka', title: 'Diagnostyka', icon: LucideIcons.Wrench, color: '#8A93A8', defaultOpen: false,
      keywords: ['testy', 'sprawdź', 'błędy'],
      items: [
        {
          id: 'diag-selftest', title: 'Self-test aplikacji', subtitle: 'Sprawdza daty (bug „północ"), pamięć, chmurę/backup i wczytanie danych — ✓/✗',
          keywords: ['self-test', 'testy', 'diagnostyka', 'sprawdź aplikację', 'daty', 'pamięć', 'backup'],
          control: { kind: 'custom', render: () => (
            <PressableScale onPress={doSelfTest} disabled={selfTestBusy} style={styles.row}>
        ...
        },
        {
          ...
          control: { kind: 'link', onPress: () => { router.push('/expenses/audit' as any); } },
        },
        {
          id: 'diag-last-crash', title: 'Pokaż ostatni błąd', subtitle: 'Po czarnym ekranie — pokaż i wyślij mi treść',
          icon: LucideIcons.Bug, accentColor: '#F59E0B',
          keywords: ['crash', 'błąd', 'awaria', 'czarny ekran', 'ostatni błąd'],
      ...
      items: [
        {
          ...
          keywords: ['wersja', 'build', 'informacje', 'sapp', 'firebase', 'framework', 'expo'],
          control: { kind: 'custom', render: () => (
            <>
...
const makeStyles = themedStyles((c: any) => StyleSheet.create({
  ...
  searchInput: { flex: 1, fontSize: 14, color: c.text.primary, paddingVertical: 9 },
  searchClear: { padding: 4 },
  searchEmpty: { ...typography.bodySmall, color: c.text.muted, textAlign: 'center', paddingVertical: spacing[6] },
  sectionTitle: {
