...
import StatTile, { metricIcon } from '@/components/dashboard/StatTile';
...
// (src/components/dashboard/StatTile.tsx, 2026-09-08) — `metricIcon` eksportowany stamtąd,
// reużywany tu tylko przez modal szczegółów kafelka (patrz import niżej).
...
export default function DashboardScreen() {
  ...
  // `fmtStat`/`fmtWave`/`unitChip`/`periodCaption` — przeniesione do
  // `src/utils/dashboard/format.ts` (2026-09-08, ekstrakcja StatTile, patrz komentarz tam) —
  // czyste funkcje bez domknięcia nad stanem tego komponentu, teraz reużywane też przez
  // `<StatTile>`.
  ...
  const renderCustomTile = (t: CustomTile): React.ReactNode => {
    if (t.type === 'stat') return (
      <TouchableOpacity activeOpacity={0.9} onPress={() => { haptic.tap(); setDetailPeriod((t.period as 'week' | 'month') ?? 'month'); setStatDetail(t); }}>
        <StatTile tile={t} statCtx={statCtx} accentColor={accentColor} cardBgDark={cardBgDark} colors={colors} s={s} updateCustomTile={updateCustomTile} pixelDayCache={pixelDayCache} />
      </TouchableOpacity>
    );
