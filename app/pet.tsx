...
export default function Pet() {
  ...
  const { name, xp, coins, setName, careTick } = usePetStore();
  const { habits, todayDone } = useHabits();
  const { entries: moodEntries } = useMoodStore();

  const [health, setHealth] = useState<{ steps: number; sleep: number }>({ steps: 0, sleep: 0 });
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.hintBox}>
          <Text style={s.hintTxt}>
            {name} karmi się tym, jak dbasz o SIEBIE — sen, ruch, nawyki i nastrój. Zadbaj o siebie, a on rośnie i świeci. Monety i sklep z ubrankami: wkrótce.
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
